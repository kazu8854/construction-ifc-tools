/**
 * Minimal viewer-visible IFC: one IfcBuildingElementProxy with IfcTriangulatedFaceSet (1m cube mesh).
 * Source: buildingSMART Sample-Test-Files — IFC 4.0.2.1 / ISO Spec - ReferenceView_V1.2 / tessellated-item.ifc
 * https://github.com/buildingSMART/Sample-Test-Files (MIT)
 */
export const VIEWER_SAFE_MINIMAL_IFC = `ISO-10303-21;
HEADER;
...
FILE_NAME(
	...
	'IFC text editor',
	'buildingSMART International - IFC text editor - 1.0.0',
...
const DEFAULT_PROJECT_LABEL = 'proxy with tessellation';

/** IFC text: single-quoted STEP strings escape ' as '' */
export function withViewerSafeProjectName(ifc: string, name: string): string {
  const safe = name.replace(/\r?\n/g, ' ').trim().slice(0, 200).replace(/'/g, "''");
  return ifc.replace(`'${DEFAULT_PROJECT_LABEL}'`, `'${safe}'`);
}

/** Heuristic: That Open / web-ifc need body geometry, not only spatial / product shells. */
export function isIfcLikelyRenderable(step: string): boolean {
  const u = step.toUpperCase();
  if (!u.includes('IFCPRODUCTDEFINITIONSHAPE')) return false;
  return (
    u.includes('IFCEXTRUDEDAREASOLID') ||
    u.includes('IFCTRIANGULATEDFACESET') ||
    u.includes('IFCFACETEDBREP') ||
    u.includes('IFCPOLYGONALFACESET')
  );
}
